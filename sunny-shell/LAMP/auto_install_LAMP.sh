...
get_pack_file (){
  package="apr-1.6.2.tar.gz  apr-util-1.6.0.tar.gz  mariadb-5.5.57-linux-x86_64.tar.gz   xcache-3.2.0.tar.gz  httpd-2.4.27.tar.bz2    php-5.6.31.tar.bz2       phpMyAdmin-4.0.10.20-all-languages.zip     wordpress-4.8-zh_CN.tar.gz"
...
#get package from http://172.18.50.75/source
  url="http://172.18.50.75/source"
...
echo "xcache has been complete configure,now resatart httpd24 and php-fpm"
/app/httpd24/bin/apachectl restart &>/dev/null && echo "Httpd24 has been restart OK,you can test now..." || echo "Something wrong when start httpd24,please check"
service php-fpm restart &>/dev/null && echo "php-fpm has been restart OK,you can test now..." || echo "Something wrong when start httpd24,please check"
